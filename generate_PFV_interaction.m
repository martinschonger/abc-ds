...
function [fig] = generate_PFV_interaction(output_root_path, exp_id, sim_id, plot_id, obstacle_poly, obstacle_polygon_cellarr, interaction_timepoints, crop_frame_idx, yz)
...
sim_path = sim_id;
[Data3, Target3, indivTrajStartIndices3, timestamps3] = recorded_trajectories_to_refdata({sim_path}, frames, "record", yz);  % FIXME: set to "eval" for 2-obstacle experiment
M3 = 2;
shift3 = shift;
...
for i = 1:frames
    tmp_plot_color = '#3392ff';
    interactive_plot_idx = 1;
    tmp_end = min(i, interactive_plot_idx(1,1));
    plt_objs3.trajectories{n} = plot(tmp_xiref3{n}(1, interactive_plot_idx:tmp_end), tmp_xiref3{n}(2, interactive_plot_idx:tmp_end), 'color', tmp_plot_color, 'linewidth', linewidth_global_major, 'displayname', '$\xi^{\mathrm{ref}}$', 'color', tmp_plot_color, 'displayname', '\ Actual traj. $\xi^{\mathrm{actual}}$');
    % if n > 1 || i > 1
    set(plt_objs3.trajectories{n}, 'handlevisibility', 'off');
    % end
    for itp = 1:size(interaction_timepoints, 1)
        if i >= interaction_timepoints(itp, 1)
            if i > interaction_timepoints(itp, 2)
                ...
                tmp_plot_color = '#ffec19';
                plt_objs3.trajectories{n} = plot(tmp_xiref3{n}(1, tmp_start_idx:tmp_end_idx), tmp_xiref3{n}(2, tmp_start_idx:tmp_end_idx), 'color', tmp_plot_color, 'linewidth', linewidth_global_major, 'displayname', '$\xi^{\mathrm{ref}}$', 'color', tmp_plot_color, 'displayname', '\ Actual traj. $\xi^{\mathrm{actual}}$');
                % if n > 1 || i > 1
                set(plt_objs3.trajectories{n}, 'handlevisibility', 'off');
                % end
                ...
                if itp < size(interaction_timepoints, 1)
                    tmp_end = min(i, interaction_timepoints(itp+1,1));
                    plt_objs3.trajectories{n} = plot(tmp_xiref3{n}(1, interactive_plot_idx:tmp_end), tmp_xiref3{n}(2, interactive_plot_idx:tmp_end), 'color', tmp_plot_color, 'linewidth', linewidth_global_major, 'displayname', '$\xi^{\mathrm{ref}}$', 'color', tmp_plot_color, 'displayname', '\ Actual traj. $\xi^{\mathrm{actual}}$');
                    % if n > 1 || i > 1
                    set(plt_objs3.trajectories{n}, 'handlevisibility', 'off');
                    % end
                else
                    tmp_end = min(i, frames);
                    plt_objs3.trajectories{n} = plot(tmp_xiref3{n}(1, interactive_plot_idx:tmp_end), tmp_xiref3{n}(2, interactive_plot_idx:tmp_end), 'color', tmp_plot_color, 'linewidth', linewidth_global_major, 'displayname', '$\xi^{\mathrm{ref}}$', 'color', tmp_plot_color, 'displayname', '\ Actual traj. $\xi^{\mathrm{actual}}$');
                    % if n > 1 || i > 1
                    set(plt_objs3.trajectories{n}, 'handlevisibility', 'off');
                    % end
    ...
    plt_objs3.trajectories{n} = plot(tmp_xiref3{n}(1, interactive_plot_idx:i), tmp_xiref3{n}(2, interactive_plot_idx:i), 'color', tmp_plot_color, 'linewidth', linewidth_global_major, 'displayname', '$\xi^{\mathrm{ref}}$', 'color', tmp_plot_color, 'displayname', '\ Actual traj. $\xi^{\mathrm{actual}}$');
    if n > 1 || i > 1
        set(plt_objs3.trajectories{n}, 'handlevisibility', 'off');
    end
    ...
    if crop_frame_idx > 0 && i >= crop_frame_idx
        break;
    end
end

   
plot([1e6, 1e6], [1e7, 1e7], 'linewidth', linewidth_global_major, 'color', '#ffec19', 'displayname', '\ Perturbation');
